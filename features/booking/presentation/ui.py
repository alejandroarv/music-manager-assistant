...
from features.booking.presentation.presenter import (
    map_booking_to_view,
)
...
def render_create_booking_form(booking_service):
    """
    Render the booking creation form and handle
    booking submission workflows.
    """
    ...
    with st.form("create_booking"):
        ...
        # Reuse shared contract-related form fields
    ...
    if submitted:
        ...
        # Delegate booking creation to the
...
def render_booking_row(view, booking_service):
    """
    Render a single booking entry and its
    associated actions.
    """
    ...
    with col1:
        ...
        # Store generated contracts in session state
        # to avoid regenerating files repeatedly
        ...
        if st.button(
            ...
            result = booking_service.generate_contract(
                view["id"]
            )
        ...
        # Display contract download once generated
    ...
    with col2:
        ...
        # Export booking as a text record
        export_data = (
            booking_service.export_single_booking(
                view["raw"]
            )
        )
...
def render_export_all(booking_service):
    """
    Render export functionality for all bookings.
    """
    ...
    if bookings:
        ...
        export_text = (
            booking_service.export_bookings_text(
                bookings
            )
        )
...
def render_booking(booking_service):
    """
    Render the complete booking management view.
    """

    st.header("Booking Manager")

    render_create_booking_form(
        booking_service
    )
